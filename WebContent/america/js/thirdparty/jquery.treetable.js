...
(function ($) {
    ...
    $.fn.treeTable = function (opts) {
        ...
        $treeTable.on("click", ".tree_head", function (event) {
            var $target = $(event.target);
            if ($target.parents("tr").attr("hasChild")) {
                expandNode($target.parents("tr").find(".tree_head"));
            }
            return false;
        });
        ...
        var expandNode = function ($target) {
            var className = $target.find("span[arrow]:eq(0)").attr('class');
            if (className == css['AN'] + ' ' + css['HLO']
                || className == css['AN'] + ' ' + css['HO']
                || className == css["AN"] + " " + css["O"]
                || className == css["AN"] + " " + css["LO"]
                ) {
                var id = $target.parents('tr[haschild]')[0].id;
                $target.find("span[arrow]:eq(0)").attr('class', css['AN'] + " " + css['HS']);

                //关闭所有孩子的tr
                shut(id);
                $target.find(".tree_node_expanded").removeClass("tree_node_collapsed");
                return;
            } else if (className == css['AN'] + ' ' + css['HLS']
                || className == css['AN'] + ' ' + css['HS']
                || className == css["AN"] + " " + css["S"]
                || className == css["AN"] + " " + css["LS"]
                ) {
                var id = $target.parents('tr')[0].id;
                $target.find("span[arrow]:eq(0)").attr('class', css['AN'] + " " + css['HO']);

                opts.beforeExpand($treeTable, id);
                //展开所有直属节点，根据图标展开子孙节点
                open(id);
                $target.find(".tree_node_expanded").addClass("tree_node_collapsed");
                return;
            }
        }
        ...
        function formatNode(tr) {
            ...
            if ($cur.attr('hasChild')) {
                ...
                prefile = "<span class=\"tree_node_expanded\"></span>";
            } else {
                var className = css['N'] + ' ' + ($cur.attr('isLastOne') ? css['LL'] : css['L']);
                prefile = "<span class=\"tree_node_leaf\"></span>";
            }
