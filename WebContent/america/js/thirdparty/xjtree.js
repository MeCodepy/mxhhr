...
;(function(window,undefined,$){
	...
	var defaults = {
        ...
        datatype: "json",
        url: false,
        cbiconpath: "images/xjtree/",
        icons: ['xj-tree-node-cb','xj-tree-node-cb xj-tree-node-cb-checked','xj-tree-node-cb xj-tree-node-cb-partchecked'],
        emptyiconpath: "",
    ...
    function __BuildNode__(treeid,nd, ht, deep, path, isend,options){
            ...
            if (deep == 1) {
                ht.push("<i class='xj-tree-icon'/>");
            }
            else if (deep > 1) {
                ht.push("<i class='xj-tree-icon' ", options.emptyiconpath, "/>");
                for (var j = 1; j < deep; j++) {
                    ht.push("<i class='xj-tree-elbow-line' ", options.emptyiconpath, "/>");
                }
            }
            ht.push("</span>");
            //img
            ...
            ht.push("<i class='xj-tree-ec-icon ", cs.join(" "), "' ", options.emptyiconpath, "/>");
            ht.push("<i class='xj-tree-node-icon' ", options.emptyiconpath, "/>");
            //checkbox
            if (options.showcheck && nd.showcheck) {
                if (nd.checkstate == null || nd.checkstate == undefined) {
                    nd.checkstate = 0;
                }
                ht.push("<i  id='", treeid, "_", nid, "_cb' class='xj-tree-node-cb' ","key='",nd.id,"' ","txt='",nd.text,"' "," />");
            }
            //a
    ...
    function __BuildEvent__(i,treeid,options){
        $(this).hover(function() {
         ...
         .find("i.xj-tree-ec-icon").each(function(e) {
    ...
    function __ExpandNode__(treeid,onlydoplus){       
        ...
        var img = $("#" + treeid + "_" + nid + " i.xj-tree-ec-icon");
        if (img.length > 0) {
            if(onlydoplus)
            {
    ...
    function __Check__(treeid,item, state, type,options){
        ...
        if (item.render && pstate != item.checkstate) {
            ...
            if (et.length == 1) {
				...
				et.attr('class', options.icons[item.checkstate]);
            }
        }
    }
