...
define("private.air.list", ['jquery', 'jcal', "city", "layer", "tool/data_iata_america", "tool/fun", "jtemplates", "private_flt_filter_init", "tool/cookie", "suggest_inter"],
    ...
    var init_suggest = function () {
        var array = new Array();
        for (var tab in iata_data.data_iata_america.gn) {
            array.push(iata_data.data_iata_america.gn[tab]);
        }
        for (var tab in iata_data.data_iata_america.gj) {
            array.push(iata_data.data_iata_america.gj[tab]);
        }
...
define("private_flt_filter_init", ["jquery", "tool/craft_type", "tool/hash_table", "layer", "tool/fun", "tool/input_valid"], function ($, c, h, l, f, valid) {
    ...
    var Init = function () {
        var btnloader = new LoadModel(null);
        ...
        $("#flight_list_result").delegate("a.bookBtn", "click", function (e) {
            ...
            var bval = $("#ipt_auth").val();

            if (Boolean(Number(bval)) === false && authCheck() === false) {
                var html = $("#box_login_popup").html();
                layer.open({
                    id:"popup_auth_box",
                    type: 1,
                    title: "登录",
                ...
                $("#popup_auth_box").find("input[name=popup_token]").val(data);
                $("#popup_auth_box").delegate("a.lg_btn5", "click", function () {
                	btnloader.loadend();
                	layer.closeAll();
                	
                	formSubmit("air/order", { token: li.find("input[name=token]").val() });
                	
                })
                $("#popup_auth_box").delegate("a.userLogina", "click", function () {
                    if (btnloader.working()) {
                        return false;
                    }
                    var acc = $.trim($(this).parent().find("input[name=userCode]").val());
                    var pwd = $.trim($(this).parent().find("input[name=userPwd]").val());
                    var code = $.trim($(this).parent().find("input[name=captcha]").val());
                    if (!valid.v_mobile(acc) && !valid.v_email(acc)) {
                        $(this).parent().find("input[name=userCode]").focus();
                        return;
                    }
                    if (!/^([a-zA-Z0-9]{6,})$/.test(pwd)) {
                        $(this).parent().find("input[name=userPwd]").focus();
                        return;
                    }
                    if (!/^([a-zA-Z0-9]{5})$/.test(code)) {
                        $(this).parent().find("input[name=captcha]").focus();
                        return;
                    }
                    btnloader = new LoadModel(this);
                    btnloader.loading();
                    var obj = this;
                    $.ajax({
                        url: "authPopup?" + Math.random(),
                        type: "POST",
                        data: { "userCode": acc, "userPwd": pwd, "captcha": code },
                        cache: false,
                        dataType: "json",
                        error: function (a, b, c) {
                            console.log(c);
                            btnloader.loadend();
                            authPopup({ "succeed": false, "msg": "请求异常" }, obj);
                        },
                        success: function (result) {
                            console.log(result);
                            btnloader.loadend();
                            authPopup(result, obj);
                        },
                        complete: function (req, status) {
                            console.log(status);
                            btnloader.loadend();
                ...
                }).delegate("img.captcha", "click", function () {
    ...
    var authPopup = function (res, obj) {
        //succeed
        if (res.succeed) {
            var data = $(obj).parent().find("input[name=popup_token]").val();
            //if (timeoutCheck(this)) { return; }
            layer.closeAll();
            formSubmit("air/order", { token: li.find("input[name=token]").val() });
        } else {//error
            $(obj).parent().find("h3").html(res.msg);
            $(obj).parent().find("input[name=userCode]").focus();
        }
    };
    var authCheck = function () {
        var succeed = false;
        $.ajax({
            url: "login/check?" + Math.random(),
            type: "POST",
            cache: false,
            async: false,//同步
            dataType: "json",
    ...
    var LoadModel = function (obj) {
        ...
        this.loading = function () {
            wait = true;
            $(obj).addClass("reg_btn_loading");
            txt = $(obj).html();
            $(obj).html("加载中");
        },
        this.loadend = function () {
            wait = false;
            $(obj).removeClass("reg_btn_loading");
            $(obj).html(txt);
        }
        return this;
    };
